...
function createFormattedArray() {
  ...
  Users.randomUserMock.forEach((userItem) => {
    ...
    item.full_name = item.name.first.concat(' ', item.name.last);
    delete item.name;
...
function filterArray(usersArray, country, minAge, maxAge, gender, favorite) {
  const filteredArray = [];
...
function searchByParameter(usersArray, param, less) {
  const findedUsers = [];
  ...
  usersArray.forEach((userItem) => {
    if (less){
      ...
      if (userItem.note.includes(paramStr)) {
        findedUsers.push(userItem);
      }
    }
  });
...
function getPercent(usersArray, param, less) {
  const findedUsers = searchByParameter(usersArray, param, less);
...
console.log('Filter by age less then 49, country: France, Germany');
const filteredArray2 = filterArray(array, ['France', 'Germany'], null, 48, null, null);
console.log(filteredArray2);

console.log('Filter by favorites, gender: female, age from 0 to 53');
...
console.log('Percent of users with age more then 30');
const percent1 = getPercent(array, 30, false);

...
require('../css/app.css');
const lab6_task1 = require('./lab6_task1')
const lab6_task2 = require('./lab6_task2')
const lab6_task4 = require('./lab6_task4')
/** ******** Your code here! *********** */
...
async function fillwithteachers(Parametres){
    ...
    try{
        await lab6_task2.CreatePieChart(array)
    }catch{
...
$(".teachers-account-info").click(function(){
    ...
    for(let i = 0 ; i < array.length ; i++){
        if(Id === array[i].id){
            ...
            lab6_task1.AddMap(array[i].coordinates)
            let days = lab6_task4.DaysBeforeNextBday(array[i].b_day)
            document.getElementById("popup-teacher-days-until-next-bday").innerHTML = 'Days until his/her next birthday: ' + days;
            if (array[i].favorite != true){
                document.getElementById("star_inf_fav").style.visibility = "hidden";
                document.getElementById("toggle-teacher-to-fav").innerHTML = "add to favorite";
                document.getElementById("toggle-teacher-to-fav").className = "add-teacher-to-fav";

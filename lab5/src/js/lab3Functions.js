...
export async function createFormattedArray(url) {
  ...
  const usersArray = [];
  _.forEach(Users, function(userItem) {
    const item = userItem;
...
export function filterArray(usersArray, country, minAge, maxAge, gender, favorite) {
  const filteredArray = [];
  _.forEach(usersArray, function(userItem) {
    let added = true;
    if (typeof (country) === 'string' || Array.isArray(country)) {
      if (typeof (country) === 'string' && userItem.country !== country) {
        ...
        _.forEach(country, function(item) {
...
export function searchByParameter(usersArray, param, less) {
  ...
  _.forEach(usersArray, function(userItem) {
    if (less == true){
      ...
      if (_.includes(userItem.full_name, paramStr) || userItem.age.toString() <= paramStr) {
